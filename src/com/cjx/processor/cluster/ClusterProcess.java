...
public class ClusterProcess {
	...
	public static void getClusterData(String word) {
		...
		try {
			...
			MessageHelper.send(data,
					String.valueOf(CommandConst.ClusterAnalysis));
			MessageHelper.send(patentsList,
					String.valueOf(CommandConst.ClusterSearchResult));
		} catch (Exception e) {
			e.printStackTrace();
			MessageHelper.send(data,
					String.valueOf(CommandConst.ClusterAnalysis));
	...
	public static void getClusterPatents(String keyWord, int pttClass,
			int cluster) {
		System.out.println("getClusterPatents:" + keyWord + "," + pttClass
				+ "," + cluster);
		...
		try {
			...
			for (int i = 0; i < hits.length(); i++) {
				PatentHitDocumentModel p = PatentHitDocumentModel.readDoc(hits
						.doc(i));
				if (p.classNumG06q.equals(getClassNumByInt(pttClass))
						&& p.cluster.equals(String.valueOf(cluster))) {
					patentsList.add(p);
				}
			}
			MessageHelper.send(patentsList,
					String.valueOf(CommandConst.ClusterItemSearch));
		} catch (Exception e) {
			e.printStackTrace();
			MessageHelper.send(patentsList,
					String.valueOf(CommandConst.ClusterItemSearch));
