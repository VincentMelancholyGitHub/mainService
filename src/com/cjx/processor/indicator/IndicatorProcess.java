	...
	public static void getTechnicalGrowthRateData(IndicatorParam param)
	{
		...
		try {
			...
			for(int i=2006; i<=2010; i++)
			{
				...
				{
					data.value11.add(new IndicatorValueItem(y, map.get(y).doubleValue()/sum));
				}
				else
				{
			...
			MessageHelper.send(data,String.valueOf(CommandConst.technicalGrowthRate));
		} catch (IOException e) {
			e.printStackTrace();
			MessageHelper.send(data, String.valueOf(CommandConst.technicalGrowthRate));
		}
	...
	public static void getTechnicalMatureRateData(IndicatorParam param)
	{
		try
		{
			...
			for(int i=2006; i<=2010; i++)
			{
				...
				{
					data.value11.add(new IndicatorValueItem(y, map11.get(y).doubleValue()/yearSum.get(y).doubleValue()));
				}
				else
				{
					data.value11.add(new IndicatorValueItem(y, 0));
			...
			MessageHelper.send(data,String.valueOf(CommandConst.technicalMatureRate));
		}
		catch(Exception e)
		{
			e.printStackTrace();
			IndicatorData nullData = new IndicatorData();
			MessageHelper.send(nullData,String.valueOf(CommandConst.technicalMatureRate));
		}
